...
function normalizeAction(raw) {
  ...
  if (Array.isArray(raw.tools) && raw.tools.length) return { tools: raw.tools };
  if (Array.isArray(raw.plan)) return { plan: raw.plan };
  if (typeof raw.tool === "string") return { tool: raw.tool, input: raw.input || {} };
  return null;
}

const READ_ONLY_TOOLS = new Set(["list_dir", "read_file", "search_code", "fetch_url"]);
...
/**
 * Ask the model once; if response is not a tool action, nudge and retry.
 */
async function requestToolAction(message, history, usageTotals, { stream = false, workdir } = {}) {
  let lastResponse = "";
...
export {
  ...
  MAX_JSON_RETRIES,
  loadSession, saveSession, sessionPath,
  ask, addUsage, normalizeAction, isReadOnlyTool, requestToolAction,
  READ_ONLY_TOOLS,
};
